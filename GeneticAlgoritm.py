...
def create_populacao(x):
    ...
    for i in range(0, x):
        ...
        x.avaliacao = contar_confrontos_rainha(x)
        population.append(x)
    return population
...
def reproduction(crossX, crossY):
    ...
    x.avaliacao = contar_confrontos_rainha(x)
    y.avaliacao = contar_confrontos_rainha(y)
    return x, y
...
def contar_confrontos_rainha(candidate):
    confrontos = 0
...
nova_populacao = create_populacao(5)
